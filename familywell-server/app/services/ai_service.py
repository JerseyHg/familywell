...
═══════════════════════════════════════════
★ 新增 recognize_text()：文字类 PDF 直接用文本模型识别
"""
import json
import base64
import logging
...
# ════════════════════════════════════════
# ★ 文字类 PDF 专用 Prompt（复用结构，去掉图片相关措辞）
# ════════════════════════════════════════
...
# ════════════════════════════════════════
# ★ 新增：文字类 PDF 识别（纯文本模型，更准确更便宜）
# ════════════════════════════════════════

async def recognize_text(text: str) -> dict:
    """
    ★ 文字类 PDF 路径：直接把提取的文字发给 LLM 文本接口。
    比视觉模型更准确、更便宜，且不受页数限制。
    """
    try:
        # 如果文本过长，截断到合理长度（避免超出 context window）
        max_chars = 15000
        ...
        # 文字类 PDF 已有原文，确保 raw_text 字段完整
        if not parsed.get("raw_text"):
            parsed["raw_text"] = text

        return parsed
    except json.JSONDecodeError as e:
        logger.error(f"AI returned invalid JSON from text: {result_text[:200]}... Error: {e}")
