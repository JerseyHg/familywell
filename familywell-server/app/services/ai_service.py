...
请仅返回JSON格式（不要包含```json标记），必须包含以下字段：
{
    ...
    // food（★ 必须估算营养值，不能为null）:
    "meal_type": "breakfast|lunch|dinner|snack",
    "food_items": [{"name": "米饭", "amount": "200g"}],
    "calories": 650,       // ★ 必填：根据食物种类和单次食用分量估算总热量
    "protein_g": 25,       // ★ 必填：估算蛋白质克数
    "fat_g": 18,           // ★ 必填：估算脂肪克数
    "carb_g": 85,          // ★ 必填：估算碳水化合物克数
    "fiber_g": 5,
    "sodium_mg": 800,
...
7. 只返回JSON，不要多余文字
8. ★★ 食物类(food)特别要求：calories、protein_g、fat_g、carb_g 必须根据食物种类和估算分量给出合理数值，绝对不能为null。即使只能大致估算也要给出数字。food_items 中每个食物必须包含 name 和 amount 字段。
9. ★★ 食物分量估算核心原则：永远按"一次合理食用量"来估算，而不是整个包装/整瓶/整罐/整袋的量。例如：
   - 速溶咖啡：按1杯冲泡量（约15g粉）估算，而不是整罐/整袋
   - 牛奶：按1杯（约250ml）估算，而不是整箱或整瓶（除非是小盒装250ml以内）
   - 饼干/零食：按合理一次食用量（如3-5片）估算，而不是整包
   - 瓶装饮料：小瓶（≤500ml）按整瓶，大瓶按1杯（约250ml）
   - 桶装/袋装食品（如麦片、奶粉）：按1份/1次用量估算
   用户拍的是食物照片记录饮食，照片中出现的包装只是用来识别食物种类，不代表用户一次吃了整个包装的量。"""
